...
pyprox = PyProx("pyprox/default.json")


@pytest.mark.asyncio
async def test_pyprox_https_request():
    response = await pyprox.config.https_request(
...
@pytest.mark.asyncio
async def test_pyprox_tcp():
    # Port 4242 is "TCP echo"
    async with pyprox.tcp_connect(host="tcpbin.com", port=4242) as tcp_conn:
...
@pytest.mark.asyncio
async def test_pyprox_tls_invalid_certificate_valid():
    async with pyprox.tls_connect(host="google.fr", port=443) as tls_conn:
...
@pytest.mark.asyncio
async def test_pyprox_tls_invalid_certificate_expired():
    with pytest.raises(ssl.SSLCertVerificationError) as excinfo:
        async with pyprox.tls_connect(host="expired.badssl.com", port=443):
...
@pytest.mark.asyncio
async def test_pyprox_tls_invalid_self_signed_certificate():
    with pytest.raises(ssl.SSLCertVerificationError) as excinfo:
        async with pyprox.tls_connect(host="self-signed.badssl.com", port=443):
            pytest.fail("TLS Connection must be closed")
...
@pytest.mark.asyncio
async def test_pyprox_tls_invalid_non_existant_certificate():
    with pytest.raises(socket.gaierror):
        async with pyprox.tls_connect(host="nonexistent.example.com", port=443):
            pytest.fail("TLS Connection should have failed but succeeded")


@pytest.mark.asyncio
async def test_pyprox_tls_letsencrypt():
    # Port 4243 is "TCP echo with TLS encryption"
    async with pyprox.tls_connect(host="tcpbin.com", port=4243) as tls_conn:
...
@pytest.mark.asyncio
async def test_pyprox_tls_lack_certificates():
...
@pytest.mark.asyncio
async def test_pyprox_tls_valid():
